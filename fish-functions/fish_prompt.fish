...
function __bobthefish_path_segment -S -a current_dir -d 'Display a shortened form of a directory'
  ...
  switch "$current_dir"
    ...
    case "$HOME/code/*/src/*"
      ...
      set directory (string sub -s (math 7 + (string length $HOME) + 4 + (string length $parent) - 2) "$current_dir")
    case "$HOME/code/*"
      set parent    (string split "/" (string sub -s (math 7 + (string length $HOME)) "$current_dir"))[1]
      set directory (string sub -s (math 7 + (string length $HOME) + (string length $parent) + 1) "$current_dir")
    ...
    case "$WHOME/code/personal/*"
      set parent    (string split "/" (string sub -s (math 28 + (string length $WUSER)) "$current_dir"))[1]
      set directory (string sub -s (math 28 + (string length $WUSER) + (string length $parent) + 1) "$current_dir")
    case "$WHOME/code/*"
