...
if (Test-Path $path) {
  Verify-Path $path;
  Write-Host "Sysinternals Suite installed." -ForegroundColor DarkGreen;
} else {
  if ($f -eq $false) {
    Write-Host "Sysinernals Suite missing! Run SysInternals-Init -f to install" -ForegroundColor Red;
  } else {
    Write-Host "Sysinternals Suite missing! Attempting to install..." -ForegroundColor DarkYellow;
    mkdir $path;
